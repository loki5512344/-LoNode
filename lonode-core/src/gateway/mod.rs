...
//!
//! Public API:
//! - [`VoiceConfig`] — credentials passed by the bot via REST.
//! - [`GatewayEvent`] — events emitted by the actor (Ready, SessionDescription, …).
//! - [`GatewayCommand`] — commands accepted by the actor (SelectProtocol, Close).
//! - [`GatewayHandle`] — handle returned by [`connect`].
//! - [`connect`] — spawn the actor task and return a handle.
